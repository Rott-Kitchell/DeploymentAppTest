...
export async function getOrderInfo(orderId) {
  const orderUrl = new URL(`https://api.bigcommerce.com/stores/${BCSTOREHASH}/v2/orders/${orderId}`),
        shippingUrl = new URL(`https://api.bigcommerce.com/stores/${BCSTOREHASH}/v2/orders/${orderId}/shipping_addresses`),
        productsUrl = new URL(`https://api.bigcommerce.com/stores/${BCSTOREHASH}/v2/orders/${orderId}/products`);

  let orderInfo = await Promise.all([await fetchJson(orderUrl, {headers} , {}), await fetchJson(shippingUrl, {headers} , {}), await fetchJson(productsUrl, {headers} , {})]).then(result => {
    let main = result.find(x => !Array.isArray(x)),
        shipping_addresses = result.find(x => Array.isArray(x)&&x[0].street_1),
        products = result.find(x => Array.isArray(x)&&x[0].product_id);
    return {...main, shipping_addresses: shipping_addresses, products: products};
  })
